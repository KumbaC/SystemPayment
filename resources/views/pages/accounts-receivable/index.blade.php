...
@section('content')
    ...
    <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <p class="text-xs uppercase tracking-wide text-gray-500">Total facturado</p>
        ...
        <div class="rounded-xl border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20">
            <p class="text-xs uppercase tracking-wide text-green-700 dark:text-green-300">Total cobrado</p>
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <form method="GET" class="flex gap-2">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar por referencia o cliente..."
    ...
    <form action="{{ route('accounts-receivable.store') }}" method="POST" class="mb-6 grid grid-cols-1 gap-3 rounded-2xl border border-gray-200 bg-white p-5 md:grid-cols-7 dark:border-gray-800 dark:bg-white/[0.03]">
        @csrf
        ...
        <select name="customer_id" class="h-10 rounded-lg border border-gray-300 px-3 text-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <tbody>
                @forelse ($invoices as $invoice)
                    ...
                    <tr class="border-t border-gray-100 align-top dark:border-gray-800">
                        ...
                        <td class="px-4 py-3">{{ $invoice->customer?->name ?? 'Consumidor Final' }}</td>
                        ...
                        <td class="px-4 py-3">
                            @if ($pending > 0)
                                <form action="{{ route('accounts-receivable.payments.store', $invoice) }}" method="POST" class="grid grid-cols-1 gap-2 md:grid-cols-4">
                                    @csrf
                                    ...
                                    <button class="h-9 rounded bg-brand-500 px-3 text-xs text-white">Abonar</button>
                                </form>
                            @else
                ...
                @empty
                    <tr><td colspan="9" class="px-4 py-8 text-center text-gray-500">Sin cuentas por cobrar registradas</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
