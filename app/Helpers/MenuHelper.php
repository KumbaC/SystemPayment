...
class MenuHelper
{
    public static function getMainNavItems(): array
    {
        $items = [
            ...
            [
                ...
                'name' => 'Ventas',
                'subItems' => [
                    ['name' => 'Listado de Ventas', 'path' => '/sales', 'permission' => 'sales.view'],
                    ['name' => 'Nueva Venta', 'path' => '/sales/create/new', 'permission' => 'sales.create'],
                ],
            ],
            [
                'icon' => 'tables',
                'name' => 'Compras',
                'subItems' => [
                    ['name' => 'Listado de Compras', 'path' => '/purchases', 'permission' => 'purchases.view'],
                    ['name' => 'Nueva Compra', 'path' => '/purchases/create/new', 'permission' => 'purchases.create'],
                ],
            ],
            [
                'icon' => 'forms',
                'name' => 'Inventario',
                'subItems' => [
                    ['name' => 'Productos', 'path' => '/products', 'permission' => 'products.view'],
                    ['name' => 'Categorías', 'path' => '/categories', 'permission' => 'products.manage'],
                ],
            ],
            [
                'icon' => 'user-profile',
            ...
            [
                ...
                'name' => 'Finanzas',
                'subItems' => [
                    ['name' => 'Cuentas por Cobrar', 'path' => '/accounts-receivable', 'permission' => 'accounts-receivable.manage'],
                    ['name' => 'Facturas por Pagar', 'path' => '/accounts-payable', 'permission' => 'accounts-payable.manage'],
                ],
            ],
            [
                'icon' => 'charts',
                'name' => 'Reportes',
                'subItems' => [
                    ['name' => 'Reporte de Ventas', 'path' => '/reports', 'permission' => 'reports.view'],
                    ['name' => 'Reporte General', 'path' => '/reports/general', 'permission' => 'reports.view'],
                ],
            ],
            [
                'icon' => 'support-ticket',
    ...
    public static function getOthersItems(): array
    {
        $items = [
            ...
            [
                ...
                'permission' => 'employee-payments.manage',
            ],
            [
                'icon' => 'pages',
                'name' => 'Respaldos',
    ...
    protected static function filterByPermission(array $items): array
    {
        $user = request()->user();
        ...
        return collect($items)->map(function ($item) use ($user) {
            if (isset($item['subItems'])) {
                $item['subItems'] = collect($item['subItems'])
                    ->filter(fn ($sub) => ! isset($sub['permission']) || ($user && $user->can($sub['permission'])))
                    ->values()
                    ->all();

                return count($item['subItems']) ? $item : null;
            }
    ...
    public static function getIconSvg($iconName): string
    {
        $icons = [
            ...
            'support-ticket' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M20 17V12C20 7.58 16.42 4 12 4S4 7.58 4 12v5M8 18h8M6.5 18.75h11" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>',
            'email' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M3 8l9 6 9-6M5 18h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" stroke="currentColor" stroke-width="1.5"/></svg>',
        ];

        return $icons[$iconName] ?? '<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';
    }
}
